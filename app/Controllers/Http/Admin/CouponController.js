...
class CouponController {
  ...
  async store ({ request, response }) {
     ...
     * 3 - clients e products - pode ser utilizado somento em produtos e clientes especificos
     * 4 - pode ser utilizado por qualquer cliente e qualquer pedido
     */
    ...
    try {
      ...
      await trx.rollback()
      return response.status(400).send({ message: 'Não foi possível criar o coupon!'})
    }
  }
  ...
  async update ({ params: { id }, request, response }) {
    ...
    try {
      ...
      await trx.rollback()
      return response.status(400).send({ message: 'Não foi possível atualizar este cupom!'})
    }
  ...
  async destroy ({ params: { id }, request, response }) {
    ...
    try {
      ...
      await trx.rollback()
      return response.status(400).send({ message: 'Não foi possível deletar este cupom!' })
    }
  }
}
